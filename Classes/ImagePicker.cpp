...
bool ImagePicker::init()
{
    ...
    // 按钮背景
    //-------------------------------------
    Sprite* sprite = Sprite::create("res/ImagePicker/bk.png");
    ...
    //-------------------------------------
    MenuItemImage* btnPhoto  = MenuItemImage::create("res/ImagePicker/ButtonPhoto.png",  "res/ImagePicker/ButtonPhoto1.png",  [=](Ref* p)
                                                     {
                                                         openPhoto();
                                                     });
    btnPhoto->setAnchorPoint(Vec2(0.5, 1));
    btnPhoto->setPosition(Vec2(sprite->getContentSize().width / 2, sprite->getContentSize().height-15));
    menu->addChild(btnPhoto);
    
    //-------------------------------------
    MenuItemImage* btnCamera = MenuItemImage::create("res/ImagePicker/ButtonCamera.png", "res/ImagePicker/ButtonCamera1.png", [=](Ref* p)
                                                     {
                                                         openCamera();
                                                     });
    btnCamera->setAnchorPoint(Vec2(0.5, 1));
    btnCamera->setPosition(btnPhoto->getPosition() + Vec2(0, -btnPhoto->getContentSize().height-15));
    menu->addChild(btnCamera);
    
    //-------------------------------------
    MenuItemImage* btnCancel = MenuItemImage::create("res/ImagePicker/ButtonCancel.png", "res/ImagePicker/ButtonCancel1.png", [=](Ref* p)
                                                     {
    ...
    // 文字
    //-------------------------------------
    Label* textPhoto  = Label::createWithSystemFont("Photo",  "", 15);
    textPhoto->setPosition(btnPhoto->getContentSize() / 2);
    textPhoto->setTextColor(Color4B::WHITE);
    btnPhoto->addChild(textPhoto);
    
    //-------------------------------------
    Label* textCamera = Label::createWithSystemFont("Camera", "", 15);
    textCamera->setPosition(btnPhoto->getContentSize() / 2);
    textCamera->setTextColor(Color4B::WHITE);
    btnCamera->addChild(textCamera);
    
    //-------------------------------------
    Label* textCancel = Label::createWithSystemFont("Cancel", "", 15);
    textCancel->setPosition(btnPhoto->getContentSize() / 2);
    textCancel->setTextColor(Color4B::RED);
